package me.karim.menu;

import me.karim.CoreExample;
import me.karim.menu.pagination.PaginatedMenu;
...
public class ButtonListener implements Listener {
        ...
        if (openMenu != null) {
            ...
            if (openMenu.getButtons().containsKey(event.getSlot())) {
                ...
                if (event.isCancelled()) {
                    Bukkit.getScheduler().runTaskLater(CoreExample.INSTANCE(), () -> player.updateInventory(), 1L);
                }
            } else {
                if ((event.getClick() == ClickType.SHIFT_LEFT || event.getClick() == ClickType.SHIFT_RIGHT)) {
                    event.setCancelled(true);
    ...
    public void onInventoryClose(InventoryCloseEvent event) {
        ...
        player.setMetadata("scanglitch", new FixedMetadataValue(CoreExample.INSTANCE(), true));
    }
    ...
    public void onPlayerMove(PlayerMoveEvent event) {
        ...
        if (player.hasMetadata("scanglitch")) {
            player.removeMetadata("scanglitch", CoreExample.INSTANCE());
